...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet, queryVariables, evidenceDict, eliminationOrder):
        ...
        for eliminationVariable in eliminationOrder:
            currentFactorsList, eliminateFactor = joinFactorsByVariable(currentFactorsList, eliminationVariable)
            if len(eliminateFactor.unconditionedVariables())<= 1:
                continue
            eliminateafter = eliminate(eliminateFactor, eliminationVariable)
            currentFactorsList.append(eliminateafter)
#            incrementallyMarginalizedJoijnt = eliminate(incrementallyMarginalizedJoint, eliminationVariable)
