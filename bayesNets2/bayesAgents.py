...
def constructBayesNet(gameState):
    ...
    for obstemp in obsVars:
        edges.append((FOOD_HOUSE_VAR,obstemp))
        edges.append((GHOST_HOUSE_VAR,obstemp))
...
def getMostLikelyFoodHousePosition(evidence, bayesNet, eliminationOrder):
    ...
    maxprob = factor.getProbability(factor.getAllPossibleAssignmentDicts()[0])
    recordass = factor.getAllPossibleAssignmentDicts()[0]
    for assignment in factor.getAllPossibleAssignmentDicts():
        if  factor.getProbability(assignment) > maxprob:
            maxprob = factor.getProbability(assignment)
            recordass = assignment
    return recordass   
...
class VPIAgent(BayesAgent):
        ...
        *Do not* take into account the "time elapsed" cost of traveling to each
        ...
        for assignment in factor.getAllPossibleAssignmentDicts():
            if assignment[FOOD_HOUSE_VAR] == TOP_LEFT_VAL and assignment[GHOST_HOUSE_VAR] == TOP_RIGHT_VAL:
                leftExpectedValue += WON_GAME_REWARD*factor.getProbability(assignment) + GHOST_COLLISION_REWARD*(1-factor.getProbability(assignment))
        for assignment in factor.getAllPossibleAssignmentDicts():
            if assignment[FOOD_HOUSE_VAR] == TOP_RIGHT_VAL and assignment[GHOST_HOUSE_VAR] == TOP_LEFT_VAL:
                rightExpectedValue += WON_GAME_REWARD*factor.getProbability(assignment) + GHOST_COLLISION_REWARD*(1-factor.getProbability(assignment))
    ...
    def computeExploreValue(self, evidence, enterEliminationOrder):
        ...
        "*** YOUR CODE HERE ***"
        for prob,explor in self.getExplorationProbsAndOutcomes(evidence):
            expectedValue += prob*max(self.computeEnterValues(explor,enterEliminationOrder))
