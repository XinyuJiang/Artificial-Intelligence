...
def main():
    ...
    for iteration in range(10000):
        ...
        graph = Graph([m, b])#first creat the graph with trainable parameter
        input_x = Input(graph, x)#got input
        input_y = Input(graph, y)#got input
        xm = MatrixMultiply(graph, input_x, m)#use trainable parameter and input,get x_0 * m_0 + x_1 * m_1 
        xm_plus_b = MatrixVectorAdd(graph, xm, b)#add b,get x_0 * m_0 + x_1 * m_1 + b
        loss = SquareLoss(graph, xm_plus_b, input_y)#compare, get the loss function
        # Then we use the graph to perform backprop and update our variables
        graph.backprop()
        graph.step(0.01)#move to reduce the loss function
...
class Graph(object):
    ...
    def __init__(self, variables):
        ...
        """
        "*** YOUR CODE HERE ***"
        self.variables = variables#a list store all variables
        #print("@variables",variables)
        self.nodeslist = []#a list store all nodes that been added to the graph
    ...
    def backprop(self):
        ...
        #reverse the list so we can operate from the end of the list
        for node in self.nodeslist[::-1]:
...
class MatrixMultiply(FunctionNode):
    ...
    def backward(inputs, gradient):
        ...
        #print ("@np.dot(inputs[1],gradient)",np.dot(inputs[1],gradient))
...
class ReLU(FunctionNode):
    ...
    def forward(inputs):
        "*** YOUR CODE HERE ***"
        return np.maximum(inputs,np.zeros_like(inputs))[0]
...
class SquareLoss(FunctionNode):
    ...
    def backward(inputs, gradient):
        ...
        #print("@inputs[1]",inputs[1])
        #print("@gradient",gradient)
        return [(inputs[0]-inputs[1])*gradient/inputs[0].size,-(inputs[0]-inputs[1])*gradient/inputs[0].size]
